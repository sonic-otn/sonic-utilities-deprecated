...
def upgrade_docker(container_name, url, cleanup_image, skip_check, tag, warm):
    ...
    if warm_configured is True or warm:
        ...
        if container_name == "swss":
            ...
            cmd = "docker exec -i swss0 orchagent_restart_check -w 2000 -r 5 " + skipPendingTaskCheck
            cmd = "docker exec -i swss1 orchagent_restart_check -w 2000 -r 5 " + skipPendingTaskCheck
            cmd = "docker exec -i swss2 orchagent_restart_check -w 2000 -r 5 " + skipPendingTaskCheck
            cmd = "docker exec -i swss3 orchagent_restart_check -w 2000 -r 5 " + skipPendingTaskCheck
    ...
    if container_name == "swss":
       container_index = ['0','1','2','3']
       for index in container_index:
           container_service_name =container_name+'@'+index 
           ...
           run_command("systemctl restart %s" % container_service_name)
           container_name = 'swss'
    else:
       run_command("docker kill %s > /dev/null" % container_name)
       run_command("docker rm %s " % container_name)
